...
pair<vector<double>, vector<double>> three_point_stencil(const vector<double>& x_arr, const vector<double>& y_arr){
    ...
    #pragma omp parallel for
    for(int i = 1; i < n-1; i++){
        dxdy[i] = (y_arr[i+1]-y_arr[i-1])/(x_arr[i+1]-x_arr[i-1]); // (f(x_(i+1))-f_(x_(i-1)))/(x_(i+1)-x_(i-1))
        truncated_x_arr[i] = x_arr[i];
    }
    return {dxdy, truncated_x_arr};
}


void save_file(vector<double> vector1, vector<double> vector2){
...
int main(){
    ...
    vector<double> y = cubic_function(logspace);
